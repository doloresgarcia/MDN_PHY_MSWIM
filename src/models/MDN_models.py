...
def get_Bhattacharyya_distance(output_dim, num_mixes):
    def mdn_losses(y_true, y_pred):
        # Reshape inputs in case this is used in a TimeDistribued layer
        y_pred = tf.reshape(y_pred, [-1, (2 * num_mixes * output_dim) + num_mixes], name='reshape_ypreds')
        y_true = tf.reshape(y_true, [-1, output_dim], name='reshape_ytrue')
        # Split the inputs into paramaters
        ...
        coll = [tfd.MultivariateNormalDiag(loc=loc, scale_diag=scale) for loc, scale
                in zip(mus, sigs)]
        mixture = tfd.Mixture(cat=cat, components=coll)
        loss = mixture.prob(y_true)
        ### implementation of the Bhattacharyya distance goes here.
        #loss = tf.reduce_mean(loss)
        return loss, mixture
...
def get_mixture_mse_acuraccy(output_dim, num_mixes):
...
def sample_from_output_layer(params, output_dim, num_mixes, temp=1.0, sigma_temp=1.0):
    ...
    """
    mus, sigs, pi_logits = split_mixture_params(params, output_dim, num_mixes)
    pis = softmax_layer(pi_logits, t=temp)
    m = sample_from_categorical(pis)
    # Alternative way to sample from categorical:
    # m = np.random.choice(range(len(pis)), p=pis)
    mus_vector = mus[m * output_dim:(m + 1) * output_dim]
...
def softmax_layer(w, t=1.0):
    ...
    """
    e = np.array(w) / t  # adjust temperature
    e -= tf.math.argmax(e)  # subtract max to protect from exploding exp values.
    e = np.exp(e)
    dist = e / np.sum(e)
    return dist
...
def plot_mdn(model,x_test):
    ...
    pis = np.apply_along_axis((lambda a: MD1.softmax(a[-N_MIXES:])), 1, y_test)
    y_samples = np.apply_along_axis(MD1.sample_from_output, 1, y_test, OUTPUT_DIMS, N_MIXES, temp=1.0, sigma_temp=1.0)
   # print('here')
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.scatter(x_test[:,0],x_test[:,1], alpha=0.5, c='r') #c=perf_down_sampled.moving
    ax.scatter(y_samples.T[0], y_samples.T[1], x_test, alpha=0.5, c='b') #c=perf_down_sampled.moving
    #ax.scatter(x_test[:,2],x_test[:,3], alpha=0.5, c='g') #c=perf_down_sampled.moving
    #ax.scatter(y_samples.T[2], y_samples.T[3], x_test, alpha=0.5, c='k') #c=perf_down_sampled.moving
    plt.show()